...
function CardForm({ card, index, onChange, onRemove, language }) {
  ...
  return (
    <div className="flex flex-col gap-3 border border-stone-100 rounded-xl p-4 bg-white">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-stone-500 block mb-1">Front</label>
          <input
            type="text"
            value={card.front}
        ...
        <div>
          <label className="text-xs text-stone-500 block mb-1">Back</label>
          <div className="flex gap-2">
            <input
              type="text"
              value={card.back}
              onChange={(e) => onChange("back", e.target.value)}
              className="flex-1 border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20"
              placeholder="Translation"
            />
            <button
              onClick={handleGenerateAudio}
              disabled={!card.back || !language || generatingAudio}
              className="px-2 py-1 border border-stone-200 rounded-lg text-stone-400 hover:text-stone-700 hover:border-stone-400 transition-colors disabled:opacity-30 text-sm"
      ...
      <div>
        <label className="text-xs text-stone-500 block mb-1">
          Romanization <span className="text-stone-300">(optional)</span>
        </label>
        <input
          type="text"
          ...
          <div>
            ...
            {card.image_url && (
              <img
                src={card.image_url}
                alt="preview"
                className="mt-2 h-20 w-20 object-cover rounded-lg border border-stone-100"
