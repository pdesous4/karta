...
function Browse() {
  ...
  const [savedDecks, setSavedDecks] = useState(new Set());
  const navigate = useNavigate();

  useEffect(() => {
    Promise.all([getPublicDecks(), getSavedDecks()])
      .then(([decksRes, savedRes]) => {
        setDecks(decksRes.data);
        setSavedDecks(new Set(savedRes.data.map((d) => d.id)));
      })
  ...
  async function handleSave(deckId) {
